...
<script>
import { operloglist,operlogclean } from '@/api/systemLog'
  ...
  methods: {
      initTableList(){ //初始化table
        if(this.searchObj.time){
          this.searchObj.beginTime=this.searchObj.time[0]+" 00:00:00"
      ...
      refresh(){ //重置
          this.searchObj.time=[formatDate(new Date()),formatDate(new Date())],//操作时间
          this.searchObj.operName='',//操作人
          this.initTableList() //初始化table
      },
      ...
      clearLog(){ //清空日志
        this.$confirm('确认清空操作日志吗?', '提示',{
            type: 'warning'
        }).then((res)=>{
          operlogclean().then(res=>{
          if(res.result){
