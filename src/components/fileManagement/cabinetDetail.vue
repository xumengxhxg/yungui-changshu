        ...
        <div v-if="list && list.length">
          <div class="clearfix">
            <div class="content" v-for="(item, index) in list" :key="index">
              <div>案件： {{item.caseName}}</div>
              <div>存放人： {{item.storerName}}</div>
              <div>存放时间： {{item.storeTime.slice(0, 10)}}</div>
              <div>
                <!-- <el-button type="primary" size="mini" class="mt10" @click="takeOut(item.infoId)">取出案卷</el-button> -->
              </div>
            </div>
          </div>
...
<script>
import { getCabinetDetail, getCabinetSpace, takeOut } from '@/api/cabinet'
export default {
  data () {
    return {
      list: [],
